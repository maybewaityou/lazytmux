...
**The out-of-tmux attach exits by stopping *inside* the suspend callback — do not move the Stop out.** `tui.Suspend` (`ui/tui.go`) wraps `tview.Application.Suspend`: when the `tmux attach` subprocess returns it calls `t.app.Stop()` *from inside* `tview.Suspend`'s callback, unconditionally (success or failure). tview's `Suspend` then sees the screen was finalized during the callback and **skips its `Resume()`**. That Resume is pure waste here — the Enter handler quits the app the instant `EnterSession` returns, so resuming would only re-enter the alt screen, hide the cursor, and enable mouse only for `Stop`/`Fini` to tear it all straight back down. That needless engage→disengage burst right at exit was confirmed (a double `EnterCA`, `\x1b[?1049h`, in the PTY stream — plain `q` does it once) to leave some real terminals (Ghostty/Alacritty/Tabby) with a screen mode half-restored, so the shell looked frozen and needed `Ctrl-C`. Stopping from inside the callback makes the attach path exit like a plain quit: one disengage, no spurious re-engagement. Stop is unconditional because a non-zero `tmux attach` exit is how tmux signals a normal session-end (e.g. exiting the last window) on some configs, not necessarily an error, and a genuine attach failure already prints its cause via tmux's own stderr. This only covers the case where `tmux attach` returns at all; if the session persists (other windows/panes alive) the subprocess blocks until the user detaches, which is correct — they are still inside tmux. The Enter handler's own `app.Stop()` after `EnterSession` is therefore a harmless no-op on this path and still required for the inside-tmux `switch-client` path (which never suspends). tview's `Suspend` carries an old warning that Stop-during-suspend can panic (tcell #440); that does not reproduce on the pinned tcell v2.13.10 (verified 30+ attach/detach cycles, no panic/hang).
...
**List ↔ details focus cycling, and the wrapper/embedded-widget trap.** → / ← cycle focus between the session list and the details pane, and ↑ / ↓ switch fields in the New/Edit form (single-line Name/Tags only — Note keeps ↑ / ↓ for its cursor). All of this lives in `handleGlobalKeys` (`ui/handlers.go`). Two non-obvious rules: (1) Focus is always queried via `listHasFocus` / `detailsHasFocus` / `searchBarHasFocus`, never a raw `GetFocus() == t.sessionList`. Each of those delegates to the widget's own `HasFocus()` rather than comparing the `*tview.Application`'s focused-primitive pointer — pointer comparison is fragile across tview's focus layers: a keyboard `SetFocus` lands on the wrapper struct, a mouse click lands on the embedded widget (`*SessionList`→`*tview.List`, `*SessionDetails`→`*tview.TextView`, `*SearchBar`→`*tview.InputField`), and as of tview v0.42.0 `InputField` further delegates mouse focus to an inner, unexported `*TextArea`. That third pointer is exactly what broke the old `searchBarHasFocus`: clicking the search bar focused the inner `*TextArea`, which matched neither the `*SearchBar` wrapper nor the embedded `*tview.InputField`, so the search bar "didn't get focus" after a click. `HasFocus` recurses through every layer uniformly, and the `*TextArea` field is unexported so it could not be named in a pointer comparison anyway. This is locked by `TestFocusQueriesUseWidgetHasFocus` in `ui/handlers_test.go`. (2) The list's own `InputCapture` (`ui/session_list.go`) returns to the search bar on ← / Backspace / Esc but deliberately NOT on →, which is reserved for the global list→details handler that runs earlier in the capture chain. `focusDetails` / `focusList` are the only focus-move entry points; the details pane does not highlight on focus, keeping its border at the resting color to match the list.
